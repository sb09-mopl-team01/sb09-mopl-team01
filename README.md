...
- `application-prod.yml`은 AWS Secrets Manager `mopl-prod-secrets`에서 DB 및 Confluent Cloud 연결 정보를 읽습니다. `KAFKA_BOOTSTRAP_SERVERS`, `KAFKA_SCHEMA_REGISTRY_URL`, `KAFKA_API_KEY`, `KAFKA_API_SECRET`, `KAFKA_SCHEMA_REGISTRY_API_KEY`, `KAFKA_SCHEMA_REGISTRY_API_SECRET`은 Git이나 Task Definition에 넣지 않습니다.
...
- Kafka와 Outbox Relay는 기본값 `KAFKA_ENABLED=false`로 비활성화됩니다. ECS prod Task는 `KAFKA_ENABLED=true`, `NOTIFICATION_DELIVERY_MODE=kafka`로 활성화합니다. 런타임은 Schema Registry에 자동 등록하지 않으며, main 배포 워크플로가 `notification-value` subject의 BACKWARD 호환성을 확인하고 스키마를 등록한 뒤 배포합니다. 발행 실패는 지수 백오프로 재시도하고 선점 만료 이벤트는 복구합니다.
...
- `mopl-prod-secrets`에 Confluent bootstrap/Schema Registry URL과 Kafka·Schema Registry API key를 등록합니다. ECS Task Role과 GitHub Actions 배포 IAM principal은 이 secret에 `secretsmanager:GetSecretValue` 권한이 필요하며, ECS security group은 Confluent bootstrap endpoint의 9092/TCP와 Schema Registry의 443/TCP outbound를 허용해야 합니다.
- 배포 workflow는 `notification-value` subject의 호환성을 `BACKWARD`로 설정하고 검증한 뒤 새 version을 등록합니다. ECS runtime은 해당 subject에 Read만 부여하고 자동 schema 등록은 하지 않습니다.
